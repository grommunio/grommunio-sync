...
if ($config_path !== false) {
    ...
    ZLog::Write(LOGLEVEL_WARN, "BackendGrammm->config file can not be found");
}

// include PHP-MAPI classes
include_once('backend/grammm/mapi/mapi.util.php');
include_once('backend/grammm/mapi/mapidefs.php');
include_once('backend/grammm/mapi/mapitags.php');
include_once('backend/grammm/mapi/mapicode.php');
include_once('backend/grammm/mapi/mapiguid.php');
...
class BackendGrammm implements IBackend, ISearchProvider {
    ...
    /**
     * Returns the BackendGrammm as it implements the ISearchProvider interface
    ...
    public function Logon($user, $domain, $pass) {
        ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->Logon(): Trying to authenticate user '%s'..", $user));
        ...
        if ($this->impersonateUser !== false) {
            ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->Logon(): Impersonation active - authenticating: '%s' - impersonating '%s'", $this->mainUser, $this->impersonateUser));
        ...
        try {
            ...
            if (mapi_last_hresult()) {
                ZLog::Write(LOGLEVEL_ERROR, sprintf("BackendGrammm->Logon(): login failed with error code: 0x%X", mapi_last_hresult()));
        ...
        if(!$this->session) {
            ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->Logon(): logon failed for user '%s'", $this->mainUser));
        ...
        if ($this->impersonateUser) {
            ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->Logon(): Impersonating user '%s'", $defaultUser));
        ...
        if($this->defaultstore === false)
            throw new AuthenticationRequiredException(sprintf("BackendGrammm->Logon(): User '%s' has no default store", $defaultUser));

        $this->store = $this->defaultstore;
        $this->storeName = $defaultUser;

        ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->Logon(): User '%s' is authenticated%s", $this->mainUser, ($this->impersonateUser ? " impersonating '".$this->impersonateUser."'" : '')));
    ...
    public function Setup($store, $checkACLonly = false, $folderid = false, $readonly = false) {
        ...
        if ($mainUser == $user && $checkACLonly && $folderid && !$this->impersonateUser) {
            ZLog::Write(LOGLEVEL_DEBUG, "BackendGrammm->Setup(): Checking ACLs for folder of the users defaultstore. Fail is forced to avoid folder duplications on mobile.");
            return false;
        }
        ...
        if ($userstore) {
            ...
            if ($checkACLonly == true) {
                ...
                if (!$folderid) {
                    if ($user != $this->mainUser) {
                        if ($this->impersonateUser) {
                            ...
                            ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->Setup(): Checking for secretary ACLs on root folder of impersonated store '%s': '%s'", $user, Utils::PrintAsString($rights)));
                        }
                        else {
                            ...
                            ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->Setup(): Checking for admin ACLs on store '%s': '%s'", $user, Utils::PrintAsString($rights)));
                        }
                    }
                    // the user has always full access to his own store
                    else {
                        $rights = true;
                        ZLog::Write(LOGLEVEL_DEBUG, "BackendGrammm->Setup(): the user has always full access to his own store");
                    }
                ...
                // check permissions on this folder
                    ...
                    ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->Setup(): Checking for '%s' ACLs on '%s' of store '%s': '%s'", ($readonly?'read':'secretary'), $folderid, $user, Utils::PrintAsString($rights)));
                    return $rights;
                }
            }
    ...
    public function GetHierarchy() {
        ...
        ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->GetHierarchy(): fetched %d folders from MAPI", count($rows)));
        ...
        foreach ($rows as $row) {
            ...
            if ((isset($row[PR_ATTR_HIDDEN]) && $row[PR_ATTR_HIDDEN]) ||
                ...
                (isset($row[PR_PARENT_SOURCE_KEY]) && $row[PR_PARENT_SOURCE_KEY] == $rootfolderprops[PR_SOURCE_KEY] && strtoupper($this->storeName) != "SYSTEM")) {
                    ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->GetHierarchy(): ignoring folder '%s' as it's a hidden/search/root folder", (isset($row[PR_DISPLAY_NAME]) ? $row[PR_DISPLAY_NAME] : "unknown")));
                    continue;
            }
            ...
            if ($folder) {
                ...
                ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->GetHierarchy(): ignoring folder '%s' as MAPIProvider->GetFolder() did not return a SyncFolder object", (isset($row[PR_DISPLAY_NAME]) ? $row[PR_DISPLAY_NAME] : "unknown")));
            }
        }

        ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->GetHierarchy(): processed %d folders, starting parent remap", count($folders)));
    ...
    public function GetImporter($folderid = false) {
        ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->GetImporter() folderid: '%s'", Utils::PrintAsString($folderid)));
        if($folderid !== false) {
            $this->importedFolders[$folderid] = $this->store;
            $wrapper = GrammmChangesWrapper::GetWrapper($this->storeName, $this->session, $this->store, $folderid, $this->storeName == $this->mainUser);
            $wrapper->Prepare(GrammmChangesWrapper::IMPORTER);
            return $wrapper;
        }
        else
    ...
    public function GetExporter($folderid = false) {
        if($folderid !== false) {
            $wrapper = GrammmChangesWrapper::GetWrapper($this->storeName, $this->session, $this->store, $folderid, $this->storeName == $this->mainUser);
            $wrapper->Prepare(GrammmChangesWrapper::EXPORTER);
            return $wrapper;
        }
        else
    ...
    public function SendMail($sm) {
        ...
        if (!(function_exists('mapi_feature') && mapi_feature('INETMAPI_IMTOMAPI'))) {
            throw new StatusException("BackendGrammm->SendMail(): ZCP/KC version is too old, INETMAPI_IMTOMAPI is not available. Install at least ZCP version 7.0.6 or later.", SYNC_COMMONSTATUS_MAILSUBMISSIONFAILED, null, LOGLEVEL_FATAL);
            return false;
        }
        $mimeLength = strlen($sm->mime);
        ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->SendMail(): RFC822: %d bytes  forward-id: '%s' reply-id: '%s' parent-id: '%s' SaveInSent: '%s' ReplaceMIME: '%s'",
        ...
        if ($mimeLength == 0) {
            throw new StatusException("BackendGrammm->SendMail(): empty mail data", SYNC_COMMONSTATUS_MAILSUBMISSIONFAILED);
        }
        ...
        if(!$outbox)
            throw new StatusException(sprintf("BackendGrammm->SendMail(): No Outbox found or unable to create message: 0x%X", mapi_last_hresult()), SYNC_COMMONSTATUS_SERVERERROR);
        ...
        ZLog::Write(LOGLEVEL_DEBUG, "BackendGrammm->SendMail(): Use the mapi_inetmapi_imtomapi function");
        ...
        if (isset($props[$sendMailProps["internetcpid"]]) && $props[$sendMailProps["internetcpid"]] != INTERNET_CPID_UTF8 && MAPIUtils::GetNativeBodyType($props) == SYNC_BODYPREFERENCE_HTML) {
            ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->SendMail(): Sent email cpid is not unicode (%d). Set it to unicode and convert email html body.", $props[$sendMailProps["internetcpid"]]));
        ...
        if (stripos($props[PR_MESSAGE_CLASS], "IPM.Schedule.Meeting.Resp.") === 0) {
            ...
            if (is_array($appointments) && !empty($appointments)) {
                ...
                if (isset($appprops[$meetingRequestProps["appSeqNr"]]) && $appprops[$meetingRequestProps["appSeqNr"]]) {
                    ...
                    ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->SendMail(): Set sequence number to:%d", $appprops[$meetingRequestProps["appSeqNr"]]));
                }
            }
        }
        ...
        if(isset($sm->source->itemid) && $sm->source->itemid) {
            ...
            if (!$this->Setup(ZPush::GetAdditionalSyncFolderStore($sm->source->folderid)))
                throw new StatusException(sprintf("BackendGrammm->SendMail() could not Setup() the backend for folder id '%s'", $sm->source->folderid), SYNC_COMMONSTATUS_SERVERERROR);
            ...
            if (isset($fwmessage) && $fwmessage) {
                ...
                if (!isset($sm->replacemime)) {
                    ...
                    if (strlen($body) > 0) {
                        ...
                        if (isset($cpid[$sendMailProps["internetcpid"]]) && $cpid[$sendMailProps["internetcpid"]] != INTERNET_CPID_UTF8) {
                            ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->SendMail(): convert plain forwarded message charset (only fw set) from '%s' to '65001'", $cpid[$sendMailProps["internetcpid"]]));
                            $fwbody = Utils::ConvertCodepageStringToUtf8($cpid[$sendMailProps["internetcpid"]], $fwbody);
                        }
                        // otherwise to the general conversion
                        else {
                            ZLog::Write(LOGLEVEL_DEBUG, "BackendGrammm->SendMail(): no charset conversion done for plain forwarded message");
                            $fwbody = w2u($fwbody);
                        }
                    ...
                    if (strlen($bodyHtml) > 0) {
                        ...
                        if (isset($cpid[$sendMailProps["internetcpid"]]) && $cpid[$sendMailProps["internetcpid"]] != INTERNET_CPID_UTF8) {
                            ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->SendMail(): convert html forwarded message charset (only fw set) from '%s' to '65001'", $cpid[$sendMailProps["internetcpid"]]));
                            $fwbodyHtml = Utils::ConvertCodepageStringToUtf8($cpid[$sendMailProps["internetcpid"]], $fwbodyHtml);
                        }
                        // otherwise to the general conversion
                        else {
                            ZLog::Write(LOGLEVEL_DEBUG, "BackendGrammm->SendMail(): no charset conversion done for html forwarded message");
                ...
                if (!isset($sm->replacemime) || $sm->replacemime == false) {
                    throw new StatusException(sprintf("BackendGrammm->SendMail(): Could not open message id '%s' in folder id '%s' to be replied/forwarded: 0x%X", $sm->source->itemid, $sm->source->folderid, mapi_last_hresult()), SYNC_COMMONSTATUS_ITEMNOTFOUND);
                }
            }
        }
        ...
        if ($hr) {
            ...
            throw new StatusException(sprintf("BackendGrammm->SendMail(): Error saving/submitting the message to the Outbox: 0x%X", $hr), $code);
        }

        ZLog::Write(LOGLEVEL_DEBUG, "BackendGrammm->SendMail(): email submitted");
        return true;
    }
    ...
    public function Fetch($folderid, $id, $contentparameters) {
        ...
        if(!$entryid)
            throw new StatusException(sprintf("BackendGrammm->Fetch('%s','%s'): Error getting entryid: 0x%X", $folderid, $sk, mapi_last_hresult()), SYNC_STATUS_OBJECTNOTFOUND);

        // open the message
        $message = mapi_msgstore_openentry($this->store, $entryid);
        if(!$message)
            throw new StatusException(sprintf("BackendGrammm->Fetch('%s','%s'): Error, unable to open message: 0x%X", $folderid, $sk, mapi_last_hresult()), SYNC_STATUS_OBJECTNOTFOUND);
    ...
    public function GetWasteBasket() {
        ...
        if (isset($storeprops[PR_IPM_WASTEBASKET_ENTRYID])) {
            ...
            if (isset($wastebasketprops[PR_SOURCE_KEY])) {
                ...
                ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->GetWasteBasket(): Got waste basket with id '%s'", $this->wastebasket));
                return $this->wastebasket;
            }
        }
        return false;
    ...
    public function GetAttachmentData($attname) {
        ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->GetAttachmentData('%s')", $attname));

        if(!strpos($attname, ":"))
            throw new StatusException(sprintf("BackendGrammm->GetAttachmentData('%s'): Error, attachment requested for non-existing item", $attname), SYNC_ITEMOPERATIONSSTATUS_INVALIDATT);
        ...
        $message = mapi_msgstore_openentry($this->store, $entryid);
        if(!$message)
            throw new StatusException(sprintf("BackendGrammm->GetAttachmentData('%s'): Error, unable to open item for attachment data for id '%s' with: 0x%X", $attname, $id, mapi_last_hresult()), SYNC_ITEMOPERATIONSSTATUS_INVALIDATT);
        ...
        if(!$attach)
            throw new StatusException(sprintf("BackendGrammm->GetAttachmentData('%s'): Error, unable to open attachment number '%s' with: 0x%X", $attname, $attachnum, mapi_last_hresult()), SYNC_ITEMOPERATIONSSTATUS_INVALIDATT);
        ...
        if(!$stream)
            throw new StatusException(sprintf("BackendGrammm->GetAttachmentData('%s'): Error, unable to open attachment data stream: 0x%X", $attname, mapi_last_hresult()), SYNC_ITEMOPERATIONSSTATUS_INVALIDATT);
    ...
    public function EmptyFolder($folderid, $includeSubfolders = true) {
        ...
        if (!$folderentryid)
            throw new StatusException(sprintf("BackendGrammm->EmptyFolder('%s','%s'): Error, unable to open folder (no entry id)", $folderid, Utils::PrintAsString($includeSubfolders)), SYNC_ITEMOPERATIONSSTATUS_SERVERERROR);
        $folder = mapi_msgstore_openentry($this->store, $folderentryid);

        if (!$folder)
            throw new StatusException(sprintf("BackendGrammm->EmptyFolder('%s','%s'): Error, unable to open parent folder (open entry)", $folderid, Utils::PrintAsString($includeSubfolders)), SYNC_ITEMOPERATIONSSTATUS_SERVERERROR);

        $flags = 0;
        if ($includeSubfolders)
            $flags = DEL_ASSOCIATED;

        ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->EmptyFolder('%s','%s'): emptying folder",$folderid, Utils::PrintAsString($includeSubfolders)));

        // empty folder!
        mapi_folder_emptyfolder($folder, $flags);
        if (mapi_last_hresult())
            throw new StatusException(sprintf("BackendGrammm->EmptyFolder('%s','%s'): Error, mapi_folder_emptyfolder() failed: 0x%X", $folderid, Utils::PrintAsString($includeSubfolders), mapi_last_hresult()), SYNC_ITEMOPERATIONSSTATUS_SERVERERROR);
    ...
    public function MeetingResponse($requestid, $folderid, $response) {
        ...
        if (!$reqentryid)
            throw new StatusException(sprintf("BackendGrammm->MeetingResponse('%s', '%s', '%s'): Error, unable to entryid of the message 0x%X", $requestid, $folderid, $response, mapi_last_hresult()), SYNC_MEETRESPSTATUS_INVALIDMEETREQ);

        $mapimessage = mapi_msgstore_openentry($this->store, $reqentryid);
        if(!$mapimessage)
            throw new StatusException(sprintf("BackendGrammm->MeetingResponse('%s','%s', '%s'): Error, unable to open request message for response 0x%X", $requestid, $folderid, $response, mapi_last_hresult()), SYNC_MEETRESPSTATUS_INVALIDMEETREQ);
        ...
        if ($folderClass != 'Email') {
            ...
            if (empty($rows)) {
                throw new StatusException(sprintf("BackendGrammm->MeetingResponse('%s','%s', '%s'): Error, meeting request not found in the inbox", $requestid, $folderid, $response), SYNC_MEETRESPSTATUS_INVALIDMEETREQ);
            }
            ZLog::Write(LOGLEVEL_DEBUG, "BackendGrammm->MeetingResponse found meeting request in the inbox");
        ...
        if(!$meetingrequest->isMeetingRequest())
            throw new StatusException(sprintf("BackendGrammm->MeetingResponse('%s','%s', '%s'): Error, attempt to respond to non-meeting request", $requestid, $folderid, $response), SYNC_MEETRESPSTATUS_INVALIDMEETREQ);

        if($meetingrequest->isLocalOrganiser())
            throw new StatusException(sprintf("BackendGrammm->MeetingResponse('%s','%s', '%s'): Error, attempt to response to meeting request that we organized", $requestid, $folderid, $response), SYNC_MEETRESPSTATUS_INVALIDMEETREQ);
        ...
        if ($deviceType == 'iphone' || $deviceType == 'ipad' || $deviceType == 'ipod') {
            ...
            if (preg_match("/^Apple-.*?\/(\d{4})\./", Request::GetUserAgent(), $matches) && isset($matches[1]) && $matches[1] >= 1607) {
                ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->MeetingResponse: iOS device %s->%s", Request::GetDeviceType(), Request::GetUserAgent()));
                $sendresponse = true;
            }
        }
        switch($response) {
        ...
        if (isset($entryid)) {
            ...
            if (!$newitem) {
                throw new StatusException(sprintf("BackendGrammm->MeetingResponse('%s','%s', '%s'): Object with entryid '%s' was not found in user's store (0x%X). It might be in a delegator's store.", $requestid, $folderid, $response, bin2hex($entryid), mapi_last_hresult()), SYNC_MEETRESPSTATUS_SERVERERROR, null, LOGLEVEL_WARN);
            }
        ...
        if ($requestid == $calendarid) {
            ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->MeetingResponse('%s','%s', '%s'): returned calender id is the same as the requestid - re-searching", $requestid, $folderid, $response));
            ...
            if (is_array($items)) {
               ...
               $calendarid = bin2hex($newprops[PR_SOURCE_KEY]);
               $calFolderId = bin2hex($newprops[PR_PARENT_SOURCE_KEY]);
               ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->MeetingResponse('%s','%s', '%s'): found other calendar entryid", $requestid, $folderid, $response));
            }

            if ($requestid == $calendarid)
                throw new StatusException(sprintf("BackendGrammm->MeetingResponse('%s','%s', '%s'): Error finding the accepted meeting response in the calendar", $requestid, $folderid, $response), SYNC_MEETRESPSTATUS_INVALIDMEETREQ);
        }
    ...
    public function HasChangesSink() {
        if (!$this->notifications) {
            ZLog::Write(LOGLEVEL_DEBUG, "BackendGrammm->HasChangesSink(): sink is not available");
            return false;
        }
        ...
        if (! $this->changesSink || mapi_last_hresult()) {
            ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->HasChangesSink(): sink could not be created with  0x%X", mapi_last_hresult()));
            return false;
        }

        $this->changesSinkHierarchyHash = $this->getHierarchyHash();
        ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->HasChangesSink(): created - HierarchyHash: %s", $this->changesSinkHierarchyHash));
    ...
    public function ChangesSinkInitialize($folderid) {
        ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->ChangesSinkInitialize(): folderid '%s'", $folderid));
    ...
    public function ChangesSink($timeout = 30) {
        ...
        if (!empty($hierarchyNotifications)) {
            ...
            if ($hash !== $this->changesSinkHierarchyHash) {
                ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->ChangesSink() Hierarchy notification, pending validation. New hierarchyHash: %s", $hash));
    ...
    public function ResolveRecipients($resolveRecipients) {
        if ($resolveRecipients instanceof SyncResolveRecipients) {
            ...
            if (isset($resolveRecipients->options)) {
                ...
                if (isset($resolveRecipientsOptions->maxambiguousrecipients) &&
                    ...
                    ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->ResolveRecipients(): The client requested %d max ambigous recipients to resolve.", $maxAmbiguousRecipients));
                }
            }
        ...
        ZLog::Write(LOGLEVEL_WARN, "BackendGrammm->ResolveRecipients(): Not a valid SyncResolveRecipients object.");
    ...
    public function GetGALSearchResults($searchquery, $searchrange, $searchpicture) {
        ...
        if (!$table)
            throw new StatusException(sprintf("BackendGrammm->GetGALSearchResults(): could not open addressbook: 0x%X", mapi_last_hresult()), SYNC_SEARCHSTATUS_STORE_CONNECTIONFAILED);
        ...
        if (mapi_last_hresult())
            throw new StatusException(sprintf("BackendGrammm->GetGALSearchResults(): could not apply restriction: 0x%X", mapi_last_hresult()), SYNC_SEARCHSTATUS_STORE_TOOCOMPLEX);
        ...
        for ($i = 0; $i < $querylimit; $i++) {
            if (!isset($abentries[$i][PR_SMTP_ADDRESS])) {
                ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->GetGALSearchResults(): The GAL entry '%s' does not have an email address and will be ignored.", w2u($abentries[$i][PR_DISPLAY_NAME])));
                continue;
            }
    ...
    */
    public function TerminateSearch($pid) {
        ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->TerminateSearch(): terminating search for pid %d", $pid));
        if (!isset($this->store) || $this->store === false) {
            ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->TerminateSearch(): The store is not available. It is not possible to remove search folder with pid %d", $pid));
            return false;
        }

        $storeProps = mapi_getprops($this->store, array(PR_STORE_SUPPORT_MASK, PR_FINDER_ENTRYID));
        if (($storeProps[PR_STORE_SUPPORT_MASK] & STORE_SEARCH_OK) != STORE_SEARCH_OK) {
            ZLog::Write(LOGLEVEL_WARN, "BackendGrammm->TerminateSearch(): Store doesn't support search folders. Public store doesn't have FINDER_ROOT folder");
            return false;
        }

        $finderfolder = mapi_msgstore_openentry($this->store, $storeProps[PR_FINDER_ENTRYID]);
        if(mapi_last_hresult() != NOERROR) {
            ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->TerminateSearch(): Unable to open search folder (0x%X)", mapi_last_hresult()));
            return false;
        }
    ...
    public function GetMAPIStoreForFolderId($store, $folderid) {
        if ($store == false) {
            ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->GetMAPIStoreForFolderId('%s', '%s'): no store specified, returning default store", $store, $folderid));
        ...
        if ($this->Setup($store, false, $folderid)) {
            ...
            ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->GetMAPIStoreForFolderId('%s', '%s'): store is not available", $store, $folderid));
            return false;
        }
    }
    ...
    public function GetUserDetails($username) {
        ZLog::Write(LOGLEVEL_WBXML, sprintf("BackendGrammm->GetUserDetails for '%s'.", $username));
    ...
    public function GetFolderStat($store, $folderid) {
        ...
        $wrapper = GrammmChangesWrapper::GetWrapper($user, false, null, $folderid, null);
        ...
        if (empty($this->folderStatCache[$user])) {
            ...
            if (count($rows) == 0) {
                ZLog::Write(LOGLEVEL_INFO, sprintf("BackendGrammm->GetFolderStat(): could not access folder statistics for user '%s'. Probably missing 'read' permissions on the root folder! Folders of this store will be synchronized ONCE per hour only!", $user));
            }
            ...
            ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->GetFolderStat() fetched status information of %d folders for store '%s'", count($this->folderStatCache[$user]), $user));
        }
    ...
    public function GetUserStoreInfo() {
        ...
        ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->GetUserStoreInfo(): user %s (%s) store size is %d bytes and contains %d folders",
    ...
    private function adviseStoreToSink($store) {
        ...
        if (!in_array($store, $this->changesSinkStores)) {
            ...
            if (mapi_last_hresult()) {
                ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->adviseStoreToSink(): failed to advised store '%s' with code 0x%X. Polling will be performed.", $store, mapi_last_hresult()));
                return false;
            }
            else {
                ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->adviseStoreToSink(): advised store '%s'", $store));
    ...
    private function openMessageStore($user) {
        ...
        if($entryid) {
            ...
            if (!$store) {
                ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->openMessageStore('%s'): Could not open store", $user));
                return false;
            }
            ...
            ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->openMessageStore('%s'): Found '%s' store: '%s'", $user, (($return_public)?'PUBLIC':'DEFAULT'),$store));
            return $store;
        }
        else {
            ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->openMessageStore('%s'): No store found for this user", $user));
            return false;
        }
    }
    ...
    public function HasSecretaryACLs($store, $folderid, $entryid = false) {
        if (!$entryid) {
            ...
            if (!$entryid) {
                ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->HasSecretaryACLs(): error, no entryid resolved for %s on store %s", $folderid, $store));
                return false;
            }
        }
        ...
        if (!$folder) {
            ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->HasSecretaryACLs(): error, could not open folder with entryid %s on store %s", bin2hex($entryid), $store));
            return false;
        }
    ...
    public function HasReadACLs($store, $folderid) {
        $entryid = mapi_msgstore_entryidfromsourcekey($store, hex2bin($folderid));
        if (!$entryid) {
            ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->HasReadACLs(): error, no entryid resolved for %s on store %s", $folderid, $store));
            return false;
        }

        $folder = mapi_msgstore_openentry($store, $entryid);
        if (!$folder) {
            ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->HasReadACLs(): error, could not open folder with entryid %s on store %s", bin2hex($entryid), $store));
            return false;
        }
        ...
        if ($oofprops != false) {
            ...
            if ($oof->oofstate == SYNC_SETTINGSOOF_GLOBAL && isset($oofprops[PR_EC_OUTOFOFFICE_FROM], $oofprops[PR_EC_OUTOFOFFICE_UNTIL])) {
                ...
                if ($now > $oofprops[PR_EC_OUTOFOFFICE_FROM] && $now > $oofprops[PR_EC_OUTOFOFFICE_UNTIL]) {
                    ...
                    ZLog::Write(LOGLEVEL_INFO, "BackendGrammm->settingsOofGet(): Out of office is set but the from and until are in the past. Disabling out of office.");
                }
                elseif ($oofprops[PR_EC_OUTOFOFFICE_FROM] < $oofprops[PR_EC_OUTOFOFFICE_UNTIL]) {
                    ...
                    ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->settingsOofGet(): Time based out of office set but end time ('%s') is before startime ('%s').",
            ...
            elseif ($oof->oofstate == SYNC_SETTINGSOOF_GLOBAL && (isset($oofprops[PR_EC_OUTOFOFFICE_FROM]) || isset($oofprops[PR_EC_OUTOFOFFICE_UNTIL]))) {
                ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->settingsOofGet(): Time based out of office set but either start time ('%s') or end time ('%s') is missing.",
            ...
            ZLog::Write(LOGLEVEL_WARN, "BackendGrammm->Unable to get out of office information");
        }
    ...
    private function settingsOofSet(&$oof) {
        ...
        if (!empty($props)) {
            ...
            $result = mapi_last_hresult();
            if ($result != NOERROR) {
                ZLog::Write(LOGLEVEL_ERROR, sprintf("BackendGrammm->settingsOofSet(): Setting oof information failed (%X)", $result));
                return false;
            }
        }
    ...
    private function settingsUserInformation(&$userinformation) {
        if (!isset($this->defaultstore) || !isset($this->mainUser)) {
            ZLog::Write(LOGLEVEL_ERROR, "BackendGrammm->settingsUserInformation(): The store or user are not available for getting user information");
            return false;
        }
        ...
        ZLog::Write(LOGLEVEL_ERROR, sprintf("BackendGrammm->settingsUserInformation(): Getting user information failed: mapi_zarafa_getuser_by_name(%X)", mapi_last_hresult()));
        return false;
    }
    ...
    */
    private function getSearchFoldersRoot() {
        ...
        $storeProps = mapi_getprops($this->store, array(PR_STORE_SUPPORT_MASK, PR_FINDER_ENTRYID));
        if(($storeProps[PR_STORE_SUPPORT_MASK] & STORE_SEARCH_OK) != STORE_SEARCH_OK) {
            ZLog::Write(LOGLEVEL_WARN, "BackendGrammm->getSearchFoldersRoot(): Store doesn't support search folders. Public store doesn't have FINDER_ROOT folder");
            return false;
        }
        ...
        if(mapi_last_hresult() != NOERROR) {
            ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->getSearchFoldersRoot(): Unable to open search folder (0x%X)", mapi_last_hresult()));
            return false;
        }
    ...
    private function getSearchRestriction($cpo) {
        ...
        if (version_compare(phpversion(),'5.3.4') < 0) {
            ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->getSearchRestriction(): Your system's PHP version (%s) might not correctly process unicode strings. Search containing such characters might not return correct results. It is recommended to update to at least PHP 5.3.4. See ZP-541 for more information.", phpversion()));
        }
    ...
    private function resolveRecipientGAL($to, $maxAmbiguousRecipients, $expandDistlist = true) {
        ZLog::Write(LOGLEVEL_WBXML, sprintf("BackendGrammm->resolveRecipientGAL(): Resolving recipient '%s' in GAL", $to));
        $addrbook = $this->getAddressbook();
        // FIXME: create a function to get the adressbook contentstable
        $ab_entryid = mapi_ab_getdefaultdir($addrbook);
        if ($ab_entryid)
        ...
        if (!$table) {
            ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->resolveRecipientGAL(): Unable to open addressbook:0x%X", mapi_last_hresult()));
            return false;
        }
        ...
        if ($querycnt > 0) {
            ...
            elseif ($querycnt > 1 && $maxAmbiguousRecipients == 0) {
                ZLog::Write(LOGLEVEL_INFO, sprintf("BackendGrammm->resolveRecipientGAL(): GAL search found %d recipients but the device hasn't requested ambiguous recipients", $querycnt));
                return $recipientGal;
            }
            ...
            for ($i = 0, $nrEntries = count($abentries); $i < $nrEntries; $i++) {
                if (strcasecmp($abentries[$i][PR_SMTP_ADDRESS], $to) !== 0 && $maxAmbiguousRecipients == 1) {
                    ZLog::Write(LOGLEVEL_INFO, sprintf("BackendGrammm->resolveRecipientGAL(): maxAmbiguousRecipients is 1 and found non-matching user (to '%s' found: '%s')", $to, $abentries[$i][PR_SMTP_ADDRESS]));
                    continue;
                }
                if ($abentries[$i][PR_OBJECT_TYPE] == MAPI_DISTLIST) {
                    // check whether to expand dist list
                    if ($expandDistlist) {
                        ZLog::Write(LOGLEVEL_DEBUG, sprintf("BackendGrammm->resolveRecipientGAL(): '%s' is a dist list. Expand it to members.", $to));
                        ...
                        for ($j = 0, $nrDistListMembers = mapi_table_getrowcount($distListContent); $j < $nrDistListMembers; $j++) {
                            ZLog::Write(LOGLEVEL_WBXML, sprintf("BackendGrammm->resolveRecipientGAL(): distlist's '%s' member", $to, $distListMembers[$j][PR_DISPLAY_NAME]));
            ...
            ZLog::Write(LOGLEVEL_WBXML, "BackendGrammm->resolveRecipientGAL(): Found a recipient in GAL");
            return $recipientGal;
        }
        else {
            ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->resolveRecipientGAL(): No recipient found for: '%s' in GAL", $to));
    ...
    private function resolveRecipientContact($to, $maxAmbiguousRecipients) {
        ZLog::Write(LOGLEVEL_WBXML, sprintf("BackendGrammm->resolveRecipientContact(): Resolving recipient '%s' in user's contacts", $to));
        ...
        if (!$root) {
            ZLog::Write(LOGLEVEL_ERROR, sprintf("BackendGrammm->resolveRecipientContact(): Unable to open default store: 0x%X", mapi_last_hresult));
        }
        ...
        if ($contacts !== false) {
            ZLog::Write(LOGLEVEL_WBXML, sprintf("BackendGrammm->resolveRecipientContact(): Found %d contacts in main contacts folder.", count($contacts)));
        ...
        if ($subfolders !== false) {
            foreach($subfolders as $folder) {
                ...
                if ($contacts !== false) {
                    ZLog::Write(LOGLEVEL_WBXML, sprintf("BackendGrammm->resolveRecipientContact(): Found %d contacts in contacts' subfolder.", count($contacts)));
                    foreach ($contacts as $contact) {
                        $recipients[] = $this->createResolveRecipient(SYNC_RESOLVERECIPIENTS_TYPE_CONTACT, $to, $contact);
                    }
                }
        ...
        if ($result == NOERROR){
                ...
                if (isset($row[PR_MDB_PROVIDER]) && $row[PR_MDB_PROVIDER] == ZARAFA_STORE_PUBLIC_GUID) {
                    ...
                    if ($subfolders !== false) {
                        foreach($subfolders as $folder) {
                            ...
                            if ($contacts !== false) {
                                ZLog::Write(LOGLEVEL_WBXML, sprintf("BackendGrammm->resolveRecipientContact(): Found %d contacts in public contacts folder.", count($contacts)));
                                foreach ($contacts as $contact) {
                                    $recipients[] = $this->createResolveRecipient(SYNC_RESOLVERECIPIENTS_TYPE_CONTACT, $to, $contact);
                                }
                            }
            ...
            ZLog::Write(LOGLEVEL_WARN, sprintf("BackendGrammm->resolveRecipientContact(): Unable to open public store: 0x%X", $result));
        }
    ...
    private function createResolveRecipient($type, $email, $recipientProperties, $recipientCount = 0) {
        ...
        if (isset($recipientProperties[$certificateProp]) && is_array($recipientProperties[$certificateProp]) && !empty($recipientProperties[$certificateProp])) {
            ...
            ZLog::Write(LOGLEVEL_INFO, sprintf("BackendGrammm->createResolveRecipient(): No certificate found for '%s' (requested email address: '%s')", $recipientProperties[PR_DISPLAY_NAME], $email));
        }
    ...
    private function getAddressbook() {
        ...
        if ($result && $this->addressbook === false) {
            ZLog::Write(LOGLEVEL_ERROR, sprintf("BackendGrammm->getAddressbook error opening addressbook 0x%X", $result));
            return false;
        }
        return $this->addressbook;
    }
