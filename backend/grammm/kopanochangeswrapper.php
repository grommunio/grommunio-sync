...
class GrammmChangesWrapper implements IImportChanges, IExportChanges {
     ...
     *
     * @access public
     * @return GrammmChangesWrapper | boolean
     */
    static public function GetWrapper($storeName, $session, $store, $folderid, $ownFolder) {
        ...
        if (isset(self::$wrappers[$storeName][$folderid]) && self::$wrappers[$storeName][$folderid]->hasDiscardDataFlag()) {
            ZLog::Write(LOGLEVEL_DEBUG, "GrammmChangesWrapper::GetWrapper(): Found existing notification check exporter. Reinitializing.");
        ...
        if (!isset(self::$wrappers[$storeName][$folderid]) && $session) {
            self::$wrappers[$storeName][$folderid] = new GrammmChangesWrapper($session, $store, $folderid, $ownFolder);
        }
        else {
            return false;
        }
     ...
     *
     * @param int $type     either GrammmChangesWrapper::IMPORTER or GrammmChangesWrapper::EXPORTER
     *
     * @access public
     * @return void
     */
        ...
        if ($this->preparedAs == self::IMPORTER) {
            if (!($this->current instanceof ImportChangesICS)) {
                ...
                if (!$this->ownFolder && !self::$backend->HasSecretaryACLs($this->store, $this->folderid)) {
                    ZLog::Write(LOGLEVEL_DEBUG, sprintf("GrammmChangesWrapper->init(): Importer: missing permissions on folderid: '%s'. Working with ReplyBackImExporter.", Utils::PrintAsString($this->folderid)));
                    $this->replyback = $this->getReplyBackImExporter();
                    $this->current = $this->replyback;
                }
                else if (!empty($this->moveSrcState)) {
                    ZLog::Write(LOGLEVEL_DEBUG, "GrammmChangesWrapper->init(): Importer: Move state available. Working with ReplyBackImExporter.");
                    $this->replyback = $this->getReplyBackImExporter();
                    $this->current = $this->replyback;
                }
            ...
            if (!($this->current instanceof ExportChangesICS)) {
                ...
                if ($this->replyback || !empty($states) || !empty($this->moveSrcState)) {
                    ZLog::Write(LOGLEVEL_DEBUG, sprintf("GrammmChangesWrapper->init(): Exporter: read-only folder with folderid: '%s'. Working with ReplyBackImExporter.", Utils::PrintAsString($this->folderid)));
                    if (!$this->replyback) {
                        $this->replyback = $this->getReplyBackImExporter();
                    }
                    $this->current = $this->replyback;
                    ...
                    if (!$this->ownFolder && !self::$backend->HasReadACLs($this->store, $this->folderid)) {
                        throw new StatusException(sprintf("GrammmChangesWrapper->init(): Exporter: missing read permissions on folderid: '%s'.", Utils::PrintAsString($this->folderid)), SYNC_STATUS_FOLDERHIERARCHYCHANGED);
                    }
            ...
            throw new FatalNotImplementedException("GrammmChangesWrapper->init(): GrammmChangesWrapper was not prepared as importer or exporter.");
        }
    }
    ...
    public function ImportMessageMove($id, $newfolder) {
        ...
        if (!$this->isReplyBackExporter()) {
            ...
            if (!self::$backend->HasSecretaryACLs($dststore, $newfolder)) {
                ZLog::Write(LOGLEVEL_DEBUG, sprintf("GrammmChangesWrapper->ImportMessageMove(): destination folderid '%s' is missing permissions. Switching to ReplyBackImExporter.", Utils::PrintAsString($newfolder)));
                $this->replyback = $this->getReplyBackImExporter();
                $this->current = $this->replyback;
